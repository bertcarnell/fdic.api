...
test_that("fdic_failures works", {
  ...
  {
    x <- fdic_failures$new(api_key_secret)
    field_temp <- x$get_available_fields()
    expect_equal(length(field_temp), 15)
    ...
    temp <- x$query_fdic()
    expect_equal(ncol(temp$data), 33)
    ...
    temp <- x$query_fdic()
    expect_equal(ncol(temp$data), 12)
    expect_equal(nrow(temp$data), 4112)
    expect_equal(temp$totals$count, 4112)
    expect_equal(temp$meta$total, 4112)
